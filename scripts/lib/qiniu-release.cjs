...
async function verifyEntry(entry, cacheBust) {
  const url = cacheBust ? `${entry.url}?kp_release_verify=${Date.now()}` : entry.url;
...
async function verifyEntriesWithRetry(entries, attempts = 20, delayMs = 30_000, cacheBust = true) {
  let lastError;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      for (const entry of entries) {
        process.stdout.write(`Verifying ${entry.url} (attempt ${attempt}/${attempts})\n`);
        await verifyEntry(entry, cacheBust);
      }
      return;
    } catch (error) {
      lastError = error;
...
module.exports = {
  ...
  createMultipartParts, createPartRanges, createQBoxAuthorization, createUploadToken,
  multipartBasePath, uploadEntry, refreshUrls, verifyEntriesWithRetry,
};
