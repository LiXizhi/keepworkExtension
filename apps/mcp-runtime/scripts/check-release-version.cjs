...
function decideRelease(eventName, previousValue, currentValue) {
  ...
  if (eventName === 'workflow_dispatch') return { publish: true, version: current.text, previousVersion: '' };
  ...
  if (comparison < 0) throw new Error(`MCP runtime version must increase, got ${previous.text} -> ${current.text}`);
  return { publish: comparison > 0, version: current.text, previousVersion: previous.text };
}
...
function main() {
  ...
  const packagePath = String(args['package-path'] || 'apps/mcp-runtime/package.json');
