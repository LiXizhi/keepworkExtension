...
- WASM NPL code wiki front: `GET /health` reports `webserverBase`; `POST /paracraft/register` may return `webserverRoot`; `ALL /webserver/:instance/*` proxies into batched `http_request` jobs. See the repository's `docs/paracraft-cli.md`.
